# [KEEP] 主干文件内的已知问题（#3/#7 未修待决策；#5 已在代码中缓解）

> 生成：2026-06 代码审查。这三条问题位于 `implementation.md` 标记为 **[KEEP]**
> 的主干文件（`src/unet.py`、`rectified/rectified_flow.py`）内。按"绝不改动主干以
> 隔离编码器贡献、保证 GeneFlow vs Gene2Image 对比公平"的原则，生成时**本轮未修改**——
> 但 **#5 后来已在 `rectified_flow.py:364-380` 加守卫缓解**（见下），#3/#7 仍未改。
...
## #5 DOPRI5 采样欠积分（t 未到 1.0 就退出）—— ✅ 已在代码中缓解
- 位置：`rectified/rectified_flow.py:335` `while t < 1.0 and step_count < num_steps`（循环行号 332→335）
- 原问题：自适应步长无安全余量。模型拟合差/训练早期/较弱的消融臂误差大 → dt 被缩到极小，
  步预算耗尽时在 t<1 退出，返回**半噪声图**并被 FID/SSIM 打分；曾被判为最可能**污染 RQ2/RQ3** 的一条。
- **现状（对齐代码）**：`rectified_flow.py:364-380` 已加**欠积分守卫**——循环后检测 `t < 1-1e-4`
  时打印警告，并用有界的固定 DOPRI5 步补积分到 t=1。故"返回半噪声图 / 无强制到 t=1"的情况
  **已不存在**，此条不再是 RQ2/RQ3 的活跃风险。（`EulerSolver` 类仍存在但从未被实例化。）
- 一致性说明：本文件抬头旧写的"绝不改动主干/本轮未修改"与实际不符——`rectified_flow.py`
  已被最小改动（此守卫 + FSAL 阶段修复）；但速度场与损失未动，编码器隔离仍成立。
